...
Log_kafka_consumer = KafkaConsumer(parameters.KAFKA_LOG_CLICKS_TASK_QUEUE, bootstrap_servers=parameters.KAFKA_SERVER,
...
def test_logNewsClickForUser_basic():
    ...
    record = list(db[MONGODB_CLICK_LOGS_TABLE_NAME].find().sort([('timestamp', -1)]).limit(1))[0]
